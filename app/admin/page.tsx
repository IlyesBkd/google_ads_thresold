...
export default function AdminPage() {
  ...
  const loadLogs = useCallback(async () => {
    ...
    if (response.success && response.data) {
      ...
      const mappedLogs: LogEntry[] = apiLogs.map((l: any) => ({
        ...
        message: l.message,
        type: l.type,
        timestamp: new Date(l.created_at || l.timestamp).toLocaleString('en-US', {
          month: 'short',
          day: 'numeric',
          hour: '2-digit',
          minute: '2-digit',
      ...
      >
        ...
        <div style={loginCard}>
          ...
          <p style={{ fontSize: 13, color: COLORS.textSecondary, marginBottom: 32 }}>
            Admin panel. Sessions are JWT-based.
          </p>
  ...
  const renderProducts = () => (
    <div>
      ...
      >
        <p style={{ fontSize: 14, color: COLORS.textSecondary, maxWidth: 500 }}>
          Manage threshold account products. Toggle availability, edit pricing, or create new
          offerings.
        </p>
        <button
              ...
              <tr key={product.id}>
                ...
                <td
                  style={{
                    ...
                    fontSize: 13,
                    color:
                      product.remaining <= product.lowStockAlert
                        ? COLORS.red
  ...
  const handleImport = async () => {
    ...
    setLoading(true);
    const response = await api.importCredentials(
      importProduct,
      importText.trim(),
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14 }}>
            <div>
              ...
              >
                Google Ads creation date (optional)
              </label>
              <input
                type="date"
                value={importDate}
            ...
            <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 20 }}>
              If set, promo expires 60 days after this date
            </div>
          </div>
              ...
              <tr>
                <th style={{ ...inventoryHeaderStyle, width: 40 }}>
                  <input
                    type="checkbox"
                    checked={
                      deletableIds.length > 0 && selectedCredentials.length === deletableIds.length
                    }
                ...
                {[
                  ...
                  <th key={h} style={inventoryHeaderStyle}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filteredCredentials.map((cred) => (
                <tr key={cred.id} style={{ borderBottom: `1px solid ${COLORS.border}` }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <span
                      style={{
                        fontSize: 11,
                        fontWeight: 500,
                        color: credentialStatusColor[cred.status],
                        background: `${credentialStatusColor[cred.status]}15`,
                        padding: '3px 10px',
                        borderRadius: 999,
                      }}
                    >
                  ...
                  <td
                    style={{ padding: '12px 16px', fontSize: 12, fontFamily: 'var(--font-mono)' }}
                  >
                    {cred.promoExpiresAt ? (
                      (() => {
                        ...
                        const color =
                          daysLeft < 7 ? COLORS.red : daysLeft < 20 ? COLORS.yellow : COLORS.green;
                        return (
                          <span style={{ color }}>
                  ...
                  <td
                    style={{
                      padding: '12px 16px',
                      fontSize: 12,
                      color: cred.orderId ? COLORS.primary : COLORS.textMuted,
                      fontFamily: 'var(--font-mono)',
                    }}
                  >
                    {cred.orderId || '—'}
  ...
  const reissueLink = async (orderId: string) => {
    setLoading(true);
    const response = await api.reissueDownloadLink(orderId);
    showToast(
      response.success
        ? 'New download link emailed to the customer'
        : response.error || 'Could not re-send the link'
    );
    setLoading(false);
  };
      ...
      <div>
        ...
        >
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Order ID', 'Date', 'Customer', 'Product', 'Qty', 'Amount', 'Coin', 'Status'].map(
                  (h) => (
                    <th
                      key={h}
                      style={{
            ...
            <tbody>
              {filteredOrders.map((o) => (
                ...
                >
                  ...
                  <td style={{ padding: '12px 14px', fontSize: 13, color: COLORS.textSecondary }}>
                    {getProductName(o.productId).replace(' Account', '')}
                  </td>
                  <td
                    style={{
                      padding: '12px 14px',
    ...
    return (
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          {/* Stock & Delivery */}
          <div style={cardStyle}>
            <div style={{ fontSize: 15, fontWeight: 600, color: COLORS.text, marginBottom: 20 }}>
              Stock & delivery
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSettings({ ...settings, download_validity_hours: e.target.value })
                  }
                  style={inputStyle}
                  type="number"
                />
          ...
          <div style={cardStyle}>
            ...
            <p
              style={{ margin: '0 0 18px', fontSize: 12, color: COLORS.textMuted, lineHeight: 1.5 }}
            >
              When you import new stock, waitlist customers are emailed automatically and a message
              is posted to your Telegram channel. Create a bot with @BotFather, add it as an admin
              of your channel, then paste the details here.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div>
              ...
              <textarea
                ...
                style={{ ...drawerInputStyle, minHeight: 80, resize: 'vertical' }}
              />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div>
                ...
                <select
                  value={productForm.thresholdValue}
                  onChange={(e) =>
                    setProductForm({ ...productForm, thresholdValue: e.target.value })
                  }
                  style={{ ...drawerInputStyle, fontFamily: 'var(--font-inter)' }}
                >
      ...
      <>
        ...
        <div style={drawerStyle}>
          ...
          {o.deliveredCredentials.length > 0 && (
            <div style={{ marginBottom: 28 }}>
              ...
              >
                ...
                <button
                  onClick={() =>
                    setRevealedCredentials({ ...revealedCredentials, [o.id]: !isRevealed })
                  }
                  style={{
                    padding: '4px 12px',
                    background: 'rgba(255,255,255,0.05)',
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: '4px 0',
                        borderBottom:
                          i < o.deliveredCredentials.length - 1
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {actionBtn('Mark delivered', COLORS.green, () =>
              updateOrderStatus(o.id, 'delivered', 'deliver')
            )}
            {actionBtn('Mark paid', COLORS.primary, () => updateOrderStatus(o.id, 'paid'))}
            {actionBtn('Re-deliver (.txt)', '#8E24AA', () => reissueLink(o.id))}
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.bg, fontFamily: 'var(--font-inter)' }}>
      ...
      <div data-main-content="" style={{ marginLeft: 248, minHeight: '100vh' }}>
        ...
        />
        <main style={{ maxWidth: 1180, padding: 28 }}>{renderPage()}</main>
      </div>
