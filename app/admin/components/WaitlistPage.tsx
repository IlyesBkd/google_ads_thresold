...
export default function WaitlistPage({
  ...
  const statsByProduct = waitlist.reduce(
    (acc, entry) => {
  ...
  const collectedEmails = Array.from(
    new Map(
      filteredWaitlist.filter((e) => e.email).map((e) => [e.email!.toLowerCase(), e.email!])
    ).values()
  );
  ...
  const viaBadge = (via: string | null) => {
    ...
    return <span style={{ fontSize: 12, color: COLORS.textSecondary }}>{labels[via] || via}</span>;
  };
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <span
                        style={{
                          fontFamily: 'var(--font-mono)',
                          fontSize: 12,
                          color: entry.telegram_username ? COLORS.primary : COLORS.textMuted,
                        }}
                      >
