...
export default function DashboardPage({
  ...
  const statCards = [
    {
      label: 'Revenue today',
      value: `$${(stats.revenueToday / 100).toFixed(0)}`,
      delta: '',
      deltaColor: COLORS.textSecondary,
    },
    {
      label: 'Revenue this week',
      value: `$${(stats.revenueWeek / 100).toLocaleString()}`,
      delta: '',
      deltaColor: COLORS.textSecondary,
    },
    {
      label: 'Revenue this month',
      value: `$${(stats.revenueMonth / 100).toLocaleString()}`,
      delta: '',
      deltaColor: COLORS.textSecondary,
    },
    {
      label: 'Sales (30d)',
      value: String(stats.sales30d),
      delta: '',
      deltaColor: COLORS.textSecondary,
    },
    {
      ...
      {lowStockProducts.length > 0 && (
        ...
        >
          <span style={{ fontSize: 14, color: COLORS.yellow }}>
            {lowStockProducts
              .map((p) => `${p.name} has only ${p.remaining} accounts left`)
              .join(' | ')}
          </span>
          <button
      ...
      {lowStockProducts.length === 0 && orders.filter((o) => o.status === 'paid').length > 0 && (
        ...
        >
          <span style={{ fontSize: 14, color: COLORS.primary }}>
            {orders.filter((o) => o.status === 'paid').length} paid orders awaiting delivery
          </span>
          <button
                ...
                <div style={{ height: 8, background: 'rgba(255,255,255,0.06)', borderRadius: 4 }}>
                  <div
                    style={{
                      width: `${pct}%`,
                      height: '100%',
                      background: pct <= 30 ? COLORS.yellow : COLORS.primary,
                      borderRadius: 4,
                    }}
                  />
                </div>
